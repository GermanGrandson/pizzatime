...
class GooglePlaces
  ...
  def find_places(lat, lng)
    response = HTTParty.get("https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=#{lat},#{lng}&radius=300&type=restaurant&keyword=pizza&rankBy=distance&key=#{ENV['google_key']}")
    sleep(1)
  ...
  def geocode(address)
    response = HTTParty.get("https://maps.googleapis.com/maps/api/geocode/json?address=#{address.gsub(' ','+')}&key=#{ENV['google_key']}")
    sleep(1)
    data = JSON.parse(response.body)['results']
    location = []
